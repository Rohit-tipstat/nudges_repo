...
def remove_outliers_iqr_specific_columns(data: pd.DataFrame, columns: List[str]) -> pd.DataFrame:
    ...
    """
    # Parse 'Login Date' to datetime
    if 'Login Date' in data.columns:
        data.loc[:, 'Login Date'] = pd.to_datetime(data['Login Date'], format='%Y-%m-%d', errors='coerce')
    ...
    filtered_data.to_csv('/home/ubuntu/api_for_nudges/output.csv', index=False, header=True)
